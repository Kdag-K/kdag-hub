...
NODEHOST="172.165.8.170"        # Node IP
...
dogye --keystore $CONFIGDIR/keystore keys generate \
    --prefix $PREFIX \
    --min-suffix $MINACCT \
    --max-suffix $ACCTCNT \
    $VERBOSE
...
dogye --keystore $CONFIGDIR/keystore transactions solo -v \
    --faucet $FAUCET \
    --accounts $ACCTS \
    --count $TRANSCNT \
    --output $TRANSFILE \
...
# dogye --monet-data-dir $CONFIGDIR transactions solo -v \
 #   --faucet $FAUCET \
#    --accounts $ACCTS \
 #   --count $TRANSCNT \
#    --output $TRANSFILE \
